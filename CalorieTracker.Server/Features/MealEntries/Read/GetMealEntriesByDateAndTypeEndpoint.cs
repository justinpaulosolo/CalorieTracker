...
public static class GetMealEntriesByDateAndTypeEndpoints
{
    ...
    {
        app.MapGet("api/meal-entries/{date}/{mealType}", async (DateTime date, string mealType, ISender sender, ClaimsPrincipal user) =>
        {
            var userId = user.FindFirst(ClaimTypes.NameIdentifier)!.Value;
            var query = new GetMealEntriesByDateAndTypeQuery { UserId = userId!, Date = date, MealType = mealType };
            var result = await sender.Send(query);
            return Results.Ok(result);
        }).WithTags("MealEntry").RequireAuthorization();
    }
}