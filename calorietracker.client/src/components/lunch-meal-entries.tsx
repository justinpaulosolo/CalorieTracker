...
import { useLunchMealEnties } from '@/utils/services/meal-entries-services';
import { MealEntries } from '@/utils/types';
  
  export default function LunchMealEntries({date}:{date:string}) {
    const { data, isLoading } = useLunchMealEnties({date});
    if (isLoading) {
      return (
